<script setup>
    ...
    const { data } = await useApi(createUrl('vistaxsegmento', {query: filtros}));
    ...
    const fetchInitTableClientes = async() => {
        try {
            const {data} = await useApi('/segmentos-inicializa-tabla');
    ...
    onMounted(async () => {
        console.log('ingresa a abrir modal');
        ...
        title.value = 'Vista por segmento: '+props.dato.nombrex;
